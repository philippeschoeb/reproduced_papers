...
def plot_training_loss(model):
    ...
    plt.text(0.7, 0.9, f'Final $\\mathcal{{L}}_{{LDA}} = {final_loss:.4f}$\n $\\min \\mathcal{{L}}_{{LDA}} = {min_loss:.4f} \\; (epoch = {min_epoch})$', transform=plt.gca().transAxes, fontsize=12,   bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))
    
    plt.tight_layout()
    plt.show()

def plot_classification_map(model, X_train, y_train, X_test=None, y_test=None, x_lim=None, y_lim=None, resolution=200,  n_shots=None, figsize=(10, 8)):
    """
    ...
    epsilon = 1e-10
    entropy = -(prob_class_0 * np.log2(prob_class_0 + epsilon) + prob_class_1 * np.log2(prob_class_1 + epsilon))
    ...
    im = ax.contourf(X_grid, Y_grid, prob_difference, levels=np.linspace(-1, 1, 21), cmap=cmap, alpha=0.7, extend='both')
    
    # Add decision boundary (where P(class 0) = P(class 1) = 0.5)
    ax.contour(X_grid, Y_grid, prob_difference, levels=[0], colors=['black'], linewidths=2.5, linestyles='-', alpha=0.8)
    

    handles, labels = [], []
    
    # Plot training data
    train_class_0 = ax.scatter(X_train[y_train==0, 0], X_train[y_train==0, 1], c='darkred', s=80, alpha=0.9, edgecolors='white', linewidth=1.5, label='Class 0 (Train)', marker='o', zorder=5)
    train_class_1 = ax.scatter(X_train[y_train==1, 0], X_train[y_train==1, 1], c='darkblue', s=80, alpha=0.9, edgecolors='white', linewidth=1.5, label='Class 1 (Train)', marker='o', zorder=5)
    
    # Plot test data if provided
    if X_test is not None and y_test is not None:
        test_class_0 = ax.scatter(X_test[y_test==0, 0], X_test[y_test==0, 1], c='darkred', s=100, alpha=0.9, edgecolors='white', linewidth=2, label='Class 0 (Test)', marker='s', zorder=5)
        test_class_1 = ax.scatter(X_test[y_test==1, 0], X_test[y_test==1, 1], c='darkblue', s=100, alpha=0.9, edgecolors='white', linewidth=2, label='Class 1 (Test)', marker='s', zorder=5)
    ...
    legend_elements = [
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='darkred', markersize=10, markeredgecolor='white', markeredgewidth=1.5, label='Class 0 (Train)', linestyle='None'),
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='darkblue', markersize=10, markeredgecolor='white', markeredgewidth=1.5, label='Class 1 (Train)', linestyle='None'),
    ]
    
    if X_test is not None and y_test is not None:
        legend_elements.extend([
            plt.Line2D([0], [0], marker='s', color='w', markerfacecolor='darkred', markersize=10, markeredgecolor='white', markeredgewidth=2, label='Class 0 (Test)', linestyle='None'),
            plt.Line2D([0], [0], marker='s', color='w', markerfacecolor='darkblue', markersize=10, markeredgecolor='white', markeredgewidth=2, label='Class 1 (Test)', linestyle='None'),
        ])
    ...
    ax.legend(handles=legend_elements, loc='upper right', framealpha=0.9, fontsize=11, ncol=3)
    
    # Add colorbar
    cbar = plt.colorbar(im, ax=ax, shrink=0.8, aspect=20)
    cbar.set_ticks([-1, -0.5, 0, 0.5, 1])
    cbar.set_ticklabels(['class 0\n(Certain)', 'class 0\n(Likely)', 'Uncertain', 'class 1\n(Likely)', 'class 1\n(Certain)'])
...
def plot_probability_axis(model, X_train, y_train, X_test=None, y_test=None, offset=0):
    ...
    # Training data
    ax.scatter(train_p_01[y_train==0], np.ones(sum(y_train==0))*offset, c='red', s=60, marker='o', alpha=0.7, label='Class 0 (Train)')
    ax.scatter(train_p_01[y_train==1], np.ones(sum(y_train==1))*(-offset), c='blue', s=60, marker='o', alpha=0.7, label='Class 1 (Train)')
    
    # Test data
    if X_test is not None:
        test_p_01 = model.get_quantum_features(X_test)[:, 0]
        ax.scatter(test_p_01[y_test==0], np.zeros(sum(y_test==0))*offset, c='red', s=80, marker='s', alpha=0.7, label='Class 0 (Test)')
        ax.scatter(test_p_01[y_test==1], np.zeros(sum(y_test==1))*(-offset), c='blue', s=80, marker='s', alpha=0.7, label='Class 1 (Test)')
