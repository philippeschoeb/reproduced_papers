...
def parse_args() -> argparse.Namespace:
    ...
    src.add_argument("--from-runs", action="store_true", help="Load histories from run directories")
    src.add_argument("--from-json", type=Path, help="Load precomputed combined history JSON")

    p.add_argument("--teacher", type=Path, help="Run dir with history_teacher.json")
    p.add_argument("--scratch", type=Path, help="Run dir with history_student_scratch.json")
    ...
    p.add_argument("--qrkd-simple", type=Path, help="Run dir with history_student_qrkd.json (simple backend)")
    p.add_argument("--qrkd-merlin", type=Path, help="Run dir with history_student_qrkd.json (merlin backend)")
    p.add_argument("--qrkd-qiskit", type=Path, help="Run dir with history_student_qrkd.json (qiskit backend)")
...
def main() -> None:
    ...
    if args.from_json:
        ...
        for name, (run_dir, stem) in runs.items():
            ...
            if test_acc:
                plt.plot(range(1, len(test_acc) + 1), test_acc, linestyle="-", label=f"{name} test", marker="o")
                plotted_any = True
        if not plotted_any:
