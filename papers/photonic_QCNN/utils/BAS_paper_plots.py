...
"""
Plot training metrics for BAS runs.
...
Usage notes:
- --input-dir must point to a run directory containing BAS_training_data_{0..N-1}.pt.
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot BAS training metrics from saved runs."
    )
    parser.add_argument(
        "--input-dir",
        required=True,
        help="Directory containing BAS_training_data_*.pt files.",
    )
    parser.add_argument(
        "--output-dir",
        default=None,
...
def _load_runs(input_dir: Path, n_runs: int) -> tuple[np.ndarray, ...]:
    ...
    for test in range(n_runs):
        path = input_dir / f"BAS_training_data_{test}.pt"
        (
            training_loss_list,
...
plt.legend()
plt.savefig(
    output_dir / "QCNN_Accuracy_BAS.pdf", dpi=120, format="pdf", bbox_inches="tight"
)
...
plt.legend()
plt.savefig(
    output_dir / "QCNN_Loss_BAS.pdf", dpi=120, format="pdf", bbox_inches="tight"
)
...
# plt.legend()
plt.savefig(
    output_dir / "QCNN_All_Acc_BAS.pdf", dpi=120, format="pdf", bbox_inches="tight"
)
