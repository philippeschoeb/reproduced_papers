...
import pytest
import torch
from perceval import BasicState, BSDistribution, StateVector
...
def _create_input_statevector(x):
    ...
    for i in range(size):
        for j in range(size):
            state_i = [0] * i + [1] + [0] * (size - i - 1)
            state_j = [0] * j + [1] + [0] * (size - j - 1)
            input_state += float(x[i][j]) * BasicState(state_i + state_j + [1, 1])
    return input_state
