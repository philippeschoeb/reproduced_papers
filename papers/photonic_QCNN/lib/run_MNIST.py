# ruff: noqa: N999
"""Experiment runner for the MNIST dataset using the MerLin implementation."""
...
def _prepare_config(overrides: dict[str, Any] | None) -> dict[str, Any]:
    cfg = copy.deepcopy(DEFAULT_MNIST_CONFIG)
    if overrides:
...
def run_experiment(
    ...
    print(
        f"Running experiment {run_id} with random state {random_state} on MNIST "
...
def run_mnist_experiments(
    ...
    results_subdir = cfg.get("results_subdir", "MNIST")
