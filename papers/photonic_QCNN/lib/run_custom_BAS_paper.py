...
def _resolve_output_dirs() -> tuple[str, str]:
    ...
    if results_env:
        ...
        results_dir = os.path.join(script_dir, "..", "results", "custom_BAS_paper")
    if models_env:
...
from photonic_QCNN.lib.src.paper_layers.HW_preserving_QCNN.Conv import (
    Conv_RBS_density_I2,
)
from photonic_QCNN.lib.src.paper_layers.HW_preserving_QCNN.Pooling import (
    Pooling_2D_density_HW,
)
from photonic_QCNN.lib.src.paper_layers.Linear_Optics import *
from photonic_QCNN.lib.src.paper_layers.measurement import *
from photonic_QCNN.lib.src.paper_layers.toolbox_basis_change import (
...
from photonic_QCNN.lib.training.training_paper import train_globally_mse  # noqa: E402
...
for test in range(nbr_test):
    ...
    save_path = os.path.join(models_dir, f"model_state_Custom_BAS_{test}")
    ...
    save_path = os.path.join(results_dir, f"Custom_BAS_training_data_{test}.pt")
...
plt.tight_layout()
plt.savefig(os.path.join(results_dir, "Custom_BAS_training_metrics.png"))
