# ruff: noqa: N999
"""Experiment runner for the Custom BAS dataset using the MerLin implementation."""
...
DEFAULT_CUSTOM_BAS_CONFIG: dict[str, Any] = {
    "n_runs": 5,
    "data_source": "paper",
    "conv_circuit": "BS",
    "dense_circuit": "BS",
...
def _prepare_config(overrides: dict[str, Any] | None) -> dict[str, Any]:
    cfg = copy.deepcopy(DEFAULT_CUSTOM_BAS_CONFIG)
    if overrides:
...
def run_experiment(
    ...
    print(
        f"Running experiment {run_id} with random state {random_state} on Custom BAS "
    ...
    if source == "paper":
        x_train, x_test, y_train, y_test = get_dataset(
            "Custom BAS", "paper", random_state
        )
    elif source == "scratch":
        x_train, x_test, y_train, y_test = get_dataset(
            "Custom BAS", "scratch", random_state
        )
    else:
...
def save_results(
    ...
    """Save results to JSON files and return summary statistics."""
...
def run_custom_bas_experiments(
    ...
    results_subdir = cfg.get("results_subdir", "custom_BAS")
    dataset_display_name = "Custom BAS"
