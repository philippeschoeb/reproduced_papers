...
def _resolve_output_dirs() -> tuple[str, str]:
    ...
    if results_env:
        ...
        results_dir = os.path.join(script_dir, "..", "results", "BAS_paper")
    if models_env:
...
from photonic_QCNN.lib.training.training_paper import train_globally_mse
...
for test in range(nbr_test):
    ...
    save_path = os.path.join(models_dir, f"model_state_BAS_{test}")
    ...
    save_path = os.path.join(results_dir, f"BAS_training_data_{test}.pt")
...
plt.tight_layout()
plt.savefig(os.path.join(results_dir, "BAS_training_metrics.png"))
