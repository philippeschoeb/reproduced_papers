...
def looks_like_qcnn_root(p):
    return os.path.isfile(os.path.join(p, "Benchmarking.py")) and os.path.isdir(
        os.path.join(p, "Result")
    )


REPO_QCNN_DIR = (
    CANDIDATE_A
    if looks_like_qcnn_root(CANDIDATE_A)
    else (CANDIDATE_B if looks_like_qcnn_root(CANDIDATE_B) else CANDIDATE_A)
)
...
def main():
    p = argparse.ArgumentParser(
        description="General QCNN experiment runner (uses QCNN/Benchmarking.py)"
    )
    p.add_argument("--dataset", choices=["mnist", "fashion_mnist"], default="mnist")
    p.add_argument(
        "--classes", default="0,1", help="e.g., '0,1', 'even,odd', '<5', '>4'"
    )
    p.add_argument(
        "--ansatz", default="1", help="1..8, 9a, 9b or unitary name (U_TTN, U_SU4, ...)"
    )
    p.add_argument(
        "--encoding",
        default="resize256",
        help="Encoding string used by the repo (e.g., resize256, pca8, autoencoder8)",
    )
    ...
    if args.encoding not in ALLOWED_ENCODINGS:
        print(
            f"Warning: encoding '{args.encoding}' not in known list; proceeding anyway. (Repo may still support it.)"
        )
    ...
    if args.ansatz not in ANSATZ_MAP:
        raise SystemExit(
            f"Unknown ansatz '{args.ansatz}'. Use 1..8, 9a, 9b or a unitary name present in the repo."
        )
    ...
    start_size = (
        os.path.getsize(REPO_RESULTS_FILE) if os.path.exists(REPO_RESULTS_FILE) else 0
    )
    ...
    OUT_SUMMARY = os.path.join(
        REPO_QCNN_DIR, "Result", f"result_ansatz{ansatz_label}.txt"
    )
    ...
    print(
        f"Seeds: {args.seeds} | Circuit: {args.circuit} | Cost: {args.cost_fn} | Binary: {binary_flag}"
    )
    ...
    if os.path.exists(REPO_RESULTS_FILE):
        with open(REPO_RESULTS_FILE, encoding="utf-8") as f:
            f.seek(start_size, io.SEEK_SET)
            new_text = f.read()

    accs = []
    pat = re.compile(
        rf"Accuracy\s+for\s+{re.escape(unitary_name)}\s+{re.escape(args.encoding)}\s*:(.+)"
    )
