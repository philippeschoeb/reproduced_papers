...
class PaperDataset:
    ...
    def plot_samples(self, split: str = "train", figsize: Tuple[int, int] = (8, 6)) -> None:
...
class CirclesDataset(PaperDataset):
    ...
    def __init__(self, n_train: int = 400, n_test: int = 100, factor: float = 0.6, noise: float = 0.05):
        ...
        super().__init__(X_train=X_train,y_train=y_train,X_test=X_test,y_test=y_test,name="Circles")
        self.factor = factor
        self.noise = noise
...
class MoonsDataset(PaperDataset):
    ...
    def __init__(self, n_train: int = 400, n_test: int = 100, noise: float = 0.1):
        ...
        super().__init__(X_train=X_train,y_train=y_train,X_test=X_test,y_test=y_test,name="Moons")
        self.noise = noise
...
class TetrominoDataset(PaperDataset):
    ...
    def __init__(self, n_train: int = 400, n_test: int = 100, noise: float = 0.05, invert_prob: float = 0.5):
        
        # Generate training and test data
        X_train, y_train_str = self._make_tetromino_dataset(n_train, noise=noise, invert_prob=invert_prob)
        X_test, y_test_str = self._make_tetromino_dataset(n_test, noise=noise, invert_prob=invert_prob)
    ...
    def _make_tetromino_dataset(self, n_samples: int, noise: float = 0.0, invert_prob: float = 0.5) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def plot_samples(self, split: str = "train", n_samples: int = 9, figsize: Tuple[int, int] = (8, 8)) -> None:
...
class OverheadMNISTDataset(PaperDataset):
    ...
    def __init__(self, 
        ...
        # Scan raw JPEGs
        self._X_train_raw, self._y_train_raw = self._scan_split("training", n_samples=n_train)
        self._X_test_raw, self._y_test_raw = self._scan_split("testing", n_samples=n_test)
    ...
    def _scan_split(self, folder: str, n_samples: int = None) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def plot_samples(self, split: str = "train", n_samples: int = 25, figsize: Tuple[int, int] = None) -> None:
    ...
    def plot_pca_pairs(self, split: str = "train", max_points: int = 2500) -> None:
        ...
        sns.pairplot(
            df,
            hue="label",
            plot_kws=dict(s=12, alpha=.6, linewidth=0),
