...
- `configs/cli.json` + `configs/runtime.json` — CLI definitions plus metadata telling the shared runner which callable to import (set `runner_callable`, `defaults_path`, and optionally `seed_callable` if you need custom seeding—otherwise the global `runtime_lib.seed.seed_everything` is applied automatically)
