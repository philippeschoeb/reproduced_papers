...
class ReUploadCircuitModelTwoMode:
    ...
    def __init__(self, dimension: int, num_layers: int, design: str = "AA"):
        ...
        if not (isinstance(design, str) and len(design) == 2 and all(c in "ABC" for c in design)):
...
class PercevalReuploadingClassifier(BaseEstimator, ClassifierMixin):
    ...
    # ----- optimizers -----
    def _train_cobyla(self, X, y, *, tau, n_shots, tol, rhobeg, maxiter, track):
        ...
        res = minimize(fun, init, method="COBYLA", tol=tol, callback=cb, options={"rhobeg": rhobeg, "maxiter": maxiter, "disp": False})
        self.best_params_ = res.x
        self._set_theta(res.x)

    def _train_lbfgsb(self, X, y, *, tau, n_shots, maxiter, maxfun, iprint, maxls, ftol, gtol, maxcor, max_eval, track):
        P = self.quantum_model.num_var_params
        init = np.random.uniform(0, 2*np.pi, size=P)
        self.training_history_["epochs"] = 0
        def fun(theta):
        ...
        def cb(xk):
            if track:
                self.training_history_["loss"].append(fun(xk))
                self.training_history_["epochs"] += 1
        res = minimize(fun, init, method="L-BFGS-B", jac=False, callback=cb, options={"maxiter": maxiter, "maxfun": maxfun, "iprint": iprint, "maxls": maxls, "ftol": ftol, "gtol": gtol, "maxcor": maxcor, "max_eval": max_eval})
        self.best_params_ = res.x
        self._set_theta(res.x)

    def _train_nelder(self, X, y, *, tau, n_shots, maxiter, maxfev, xatol, fatol, adaptive, track):
        P = self.quantum_model.num_var_params
        init = np.random.uniform(0, 2*np.pi, size=P)
        self.training_history_["epochs"] = 0
        def fun(theta):
        ...
        def cb(xk):
            if track:
                self.training_history_["loss"].append(fun(xk))
                self.training_history_["epochs"] += 1
        res = minimize(fun, init, method="Nelder-Mead", callback=cb, options={"maxiter": maxiter, "maxfev": maxfev, "xatol": xatol, "fatol": fatol, "adaptive": adaptive})
        self.best_params_ = res.x
        self._set_theta(res.x)
    ...
    def _train_adam_psr(self, X, y, *, tau, n_shots, max_epochs, lr, beta1, beta2, eps, shift, batch, track):
        P = self.quantum_model.num_var_params
        theta = np.random.uniform(0, 2*np.pi, size=P)
        m = np.zeros(P)
        v = np.zeros(P)
        self.training_history_["epochs"] = 0
        N = X.shape[0]
        ...
        for epoch in range(1, max_epochs+1):
            ...
            if batch < N:
                ...
                X_b, y_b = X, y
            g = self._psr_gradient(theta, X_b, y_b, tau, n_shots, shift)
            m = beta1*m + (1-beta1)*g
            ...
            if track:
                loss_epoch = self._objective(theta, X, y, tau, n_shots)
                self.training_history_["loss"].append(float(loss_epoch))
                self.training_history_["epochs"] += 1
    ...
    def fit(
        ...
        *,
        ...
        cobyla_rhobeg: float = np.pi/2,
        cobyla_maxiter: int | None = None,
        # L‑BFGS‑B
        lbfgsb_maxiter: int = 15000,
        ...
        lbfgsb_max_eval: int | None = None,
        # Nelder‑Mead
        nelder_maxiter: int | None = None,
        nelder_maxfev: int | None = None,
        nelder_xatol: float = 1e-4,
        nelder_fatol: float = 1e-4,
        nelder_adaptive: bool = False,
        # Adam + PSR
        lr: float = 1e-2,
        beta1: float = 0.9,
        beta2: float = 0.999,
        adam_eps: float = 1e-8,
        psr_shift: float = np.pi/2,
        psr_batch_size: int = 128,
        ...
        if optim == "cobyla":
            self._train_cobyla(X, y, tau=tau, n_shots=n_shots, tol=cobyla_tol, rhobeg=cobyla_rhobeg, maxiter=cobyla_maxiter or max_epochs, track=track_history)
        elif optim in {"l_bfgs_b", "lbfgsb"}:
            self._train_lbfgsb(X, y, tau=tau, n_shots=n_shots, maxiter=lbfgsb_maxiter or max_epochs, maxfun=lbfgsb_maxfun, iprint=lbfgsb_iprint, maxls=lbfgsb_maxls, ftol=lbfgsb_ftol, gtol=lbfgsb_gtol, maxcor=lbfgsb_maxcor, max_eval=lbfgsb_max_eval, track=track_history)
        elif optim == "nelder_mead":
            self._train_nelder(X, y, tau=tau, n_shots=n_shots, maxiter=nelder_maxiter or max_epochs, maxfev=nelder_maxfev, xatol=nelder_xatol, fatol=nelder_fatol, adaptive=nelder_adaptive, track=track_history)
        elif optim == "adam_psr":
            self._train_adam_psr(X, y, tau=tau, n_shots=n_shots, max_epochs=max_epochs, lr=lr, beta1=beta1, beta2=beta2, eps=adam_eps, shift=psr_shift, batch=psr_batch_size, track=track_history)
        else:
